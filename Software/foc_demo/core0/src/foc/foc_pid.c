...
ATTR_RAMFUNC float foc_pi_controller(foc_pid_contrl_t *pid, float cur, float exp)
{
    float err = exp - cur;
...
ATTR_RAMFUNC float foc_pid_controller(foc_pid_contrl_t *pid, float cur, float d, float exp)
{
    float err = exp - cur;
    pid->integral = foc_pid_limit(pid->integral + err, pid->integral_limit);
...
ATTR_RAMFUNC float foc_pid_increase_controller(foc_pid_contrl_t *pid, float cur, float exp)
{
    float err = exp - cur;
